...
try:
    ...
    print("Consumer_token = {} , consumer_secret= {} \n ".format(consumer_token,consumer_secret))
...
df = pd.read_csv("twibot-22.csv", sep=",")
print("df = {} ".format(df))
data_top = df.head()
print("datatop = {} ".format(data_top))
...
#print("\n\n DF[user-id] = {} ".format(df[0]))
for idx, user_ids in enumerate(grouper(df["user-id"], 100)):
    #print("user_ids = {} ".format(user_ids))
    # Group of user ids now looks like: (user_id0, user_id1, ..., user_id99)

    # Print progress
    if (idx % 10 == 0):
        print(str(round(idx * 100 * 100 / len(df["user-id"]))) + "% progress", end="\r", flush=True)
